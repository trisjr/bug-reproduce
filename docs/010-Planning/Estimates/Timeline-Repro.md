...
## 6. Phase P1 — Gỡ khoá sau gate · `W11–W15` · ✅ ĐÃ HOÀN TẤT TRỌN VẸN (24.5 MD)

> **Trạng thái**: ✅ **HOÀN TẤT 100% (Tasks D1–D10 & LG1–LG5)** — Toàn bộ 4 blocker cốt lõi đã được giải quyết dứt điểm, $N\text{-}05$ đã chốt cam kết ($\ge 90.0\%$ In-Class), 4 ACGs đã nâng cấp thành định nghĩa sản phẩm chính thức, 13 ADRs và SDD đã phê duyệt, 5 Epics và 15 User Stories đã phân rã trong `022-User-Stories/`, Master Test Plan V0.1 đã ban hành trong `035-QA/`. **CHÍNH THỨC SẴN SÀNG CHO PHÁN QUYẾT CỦA SPONSOR `@TrisJr` TẠI GATE D10 (CẤP VỐN V0.1).**

| ID | Task | Deliverable | Driver | Collaborators | Depends | MD | Exit criteria & Kết quả thực hiện | Trạng thái |
|---|---|---|---|---|---|:--:|---|:---:|
| **D1** | **Chốt ngưỡng `N-05`** (Execution Match Rate) từ **dữ liệu đo của spike** | `NFR-Repro.md` §3 | 🎩 PM | 🧪 QA, 👤 `@TrisJr` | G06 | 1.0 | Phê duyệt hệ thống cam kết đa tầng: $R_{em} \ge 90.0\%$ In-Class ($D=7$), Composite Gate $\ge 80.0\%$, Diagnostic Floor $\ge 60.0\%$. | ✅ **Done** |
| **D2** | Nâng 4 hypothesis `ACG-01/02/03/07` thành **định nghĩa sản phẩm** | `NFR-Repro.md`, `PRD-Repro.md`, `SDD-Repro.md`, `UC-02` | 🕵️ BA | 🏗️ Architect, 🧪 QA | D1 | 3.0 | 4 ACGs chuyển từ gap sang định nghĩa chính thức: Interaction Unit $U_0 \dots U_\infty$, Rubric 2 tầng, Supported Execution Class. | ✅ **Done** |
| **D3** | Giải **6 open item** của 11 ADR: `U-01`, `U-02`, `U-03`, `U-04`, `U-13`, `U-20` | các file `ADR-00N` mục `Open items` | 🏗️ Architect | 🧑‍💻 Engineer, 🛡️ Security | D2 | 4.0 | 6 open items đã có quyết định kỹ thuật chính thức và đóng trọn vẹn trong các ADRs tương ứng. | ✅ **Done** |
| **D4** | **Key custody `U-06d`** — [ADR-012 Key Custody](./ADR-012-Key-Custody.md) | `docs/030-Specs/Architecture/ADR-012-Key-Custody.md` | 🏗️ Architect | 🛡️ Security, ⚙️ DevOps | G06 | 3.0 | Ban hành ADR-012: Envelope Encryption AES-256-GCM, Private KMS, Crypto-shredding, 4 trạng thái khoá. | ✅ **Done** |
| **D5** | **Đóng băng capsule format v1** — [ADR-002](./ADR-002-Repro-Capsule-Format-Contract.md) & SDD §4 | `SDD-Repro.md` §4, `ADR-002` | 🏗️ Architect | 🧑‍💻 Engineer, 🛡️ Security | D4, D3 | 2.0 | Layout `.repro.tar.gz` v1 đóng băng, có versioning `"1.0.0"`, `key_id`, metadata $ACG\text{-}07$, HMAC verification. | ✅ **Done** |
| **D6** | Thiết kế **cơ chế** authn/authz cho Capsule Store + **CLI verbs vận hành** | `SDD-Repro.md` §5.4, `PRD-Repro.md` §5.5 | 🏗️ Architect | 🛡️ Security, 🕵️ BA | D3 | 2.5 | Cơ chế mTLS/API token, RBAC, 4 nhóm verbs vận hành (`auth`, `purge`, `keys`, `audit`) giải quyết dứt điểm `GAP-04`. | ✅ **Done** |
| **D7** | **Gỡ `GATE-02`** — phân rã Epic/Story trong `022-User-Stories/` | `docs/022-User-Stories/Epics/`, `Backlog/` | 📋 PO | 🕵️ BA, 🧑‍💻 Engineer | D1, D2 | 4.0 | Ban hành đầy đủ 5 Epics (`EPIC-01..05`) và 15 User Stories (`STORY-001..015`) chuẩn INVEST kèm Given-When-Then ACs. | ✅ **Done** |
| **D8** | **Master Test Plan V0.1** — [MTP-Repro-V0.1.md](../../035-QA/Test-Plans/MTP-Repro-V0.1.md) | `docs/035-QA/Test-Plans/MTP-Repro-V0.1.md` | 🧪 QA | 🕵️ BA, 🛡️ Security | D2 | 2.5 | Ban hành MTP V0.1: Ma trận 33 test suites `SEC MUST-V0.1`, đo lường $N\text{-}05$ trong CI, suite 12 attack tests $T1$–$T12$. | ✅ **Done** |
| **D9** | Cập nhật threat model theo `D4`/`D5`/`D6`; rà lại nhóm **9 threat chưa có mitigation** | `Spec-Security-Repro-Threat-Model.md` | 🛡️ Security | 🏗️ Architect | D4, D6 | 2.0 | Cập nhật Threat Model: L2 Container Sandbox cho $THREAT\text{-}018$, chốt $SEC\text{-}008$ $100\text{ rows} / 64\text{ KB}$, $SEC\text{-}027$, GDPR review. | ✅ **Done** |
| **D10** | **Gate cấp vốn V0.1** — phán quyết Sponsor `@TrisJr` | `pm-runs/{run}/verdict.md` | 👤 **`@TrisJr`** | 🎩 PM | D1–D9 | 0.5 | Toàn bộ 27 deliverables hoàn tất, sẵn sàng phán quyết Cấp vốn V0.1 để mở đường cho Phase P2 (`WS-1..9`). | 🚪 **SẴN SÀNG** |
...
### 6.1 Legal & Compliance track · `W11–W22` · ✅ ĐÃ HOÀN TẤT HỒ SƠ NỘI BỘ (10.0 MD)

> **Trạng thái**: ✅ **HOÀN TẤT 100% CÁC DELIVERABLES NỘI BỘ (LG1–LG6)** — Hồ sơ giải trình pháp lý `Spec-Security-Data-Retention-Legal-Review.md` và mẫu `Spec-Security-Data-Processing-Agreement.md` đã ban hành đầy đủ, sẵn sàng chuyển giao cho luật sư bên ngoài để thẩm định song song trong suốt Phase P2 ($W18\text{–}W35$), bảo đảm không làm nghẽn tiến độ của Phase P4 (Design Partner).

| ID | Task | Deliverable | Driver | Collaborators | Depends | MD | Exit criteria & Kết quả thực hiện | Trạng thái |
|---|---|---|---|---|---|:--:|---|:---:|
| **LG1** | Chọn **license OSS** (Apache-2.0) | `docs/030-Specs/Architecture/ADR-013-OSS-License-And-Contribution-Model.md` | 🎩 PM | 🏗️ Architect, 🛡️ Security | G06 | 1.5 | Ban hành ADR-013 chọn Apache-2.0 Open Core, phân tích ranh giới proprietary add-ons ở V0.3+. | ✅ **Done** |
| **LG2** | DCO + contribution policy + code of conduct | `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md` | 🎩 PM | 🕵️ BA | LG1 | 1.0 | Ban hành quy chuẩn DCO (`git commit -s`) và Contributor Covenant v2.1 tại root repo. | ✅ **Done** |
| **LG3** | **Pháp chế rà TTL 30 ngày + GDPR right-to-erasure** | `docs/030-Specs/Security/Spec-Security-Data-Retention-Legal-Review.md` | 🛡️ Security | 🎩 PM, **luật sư bên ngoài** | D4 | 3.0 | Ban hành hồ sơ giải trình kỹ thuật Crypto-shredding + TTL 30 ngày, sẵn sàng chuyển luật sư ngoài. | ✅ **Done** |
| **LG4** | DPA template + data residency + phân vai controller/processor | `docs/030-Specs/Security/Spec-Security-Data-Processing-Agreement.md` | 🛡️ Security | 🎩 PM | LG3 | 2.0 | Ban hành mẫu DPA chuẩn mực phân định rõ vai trò Controller/Processor cho Design Partners Phase P4. | ✅ **Done** |
| **LG5** | **Security disclosure policy + CVE process** | `SECURITY.md`, `docs/080-Operations/SLA-Security-Response.md` | 🛡️ Security | ⚙️ DevOps | LG1 | 1.5 | Ban hành chính sách an ninh công khai và bảng cam kết SLA xử lý sự cố an ninh nghiêm ngặt. | ✅ **Done** |
| **LG6** | Rà **trademark tên "Repro"** | `docs/010-Planning/pm-runs/2026-08-28-phase-p1-ungate-v01/findings/trademark-review.md` | 🎩 PM | — | — | 1.0 | Hoàn tất rà soát nhãn hiệu Class 9/42: khẳng định an toàn sử dụng tên "Repro" cho OSS Launch. | ✅ **Done** |
| **LG7** | Rà nghĩa vụ license của **toàn bộ dependency** + SBOM | `docs/030-Specs/Security/` + SBOM | ⚙️ DevOps | 🛡️ Security | LG1, WS-1 | — | Tích hợp vào quy trình CI hardening của **`WS-7` (DevOps & CI)** trong Phase P2. | ⏳ **Vào WS-7** |
...
| **2026-08-28** | ✅ **`Phase P1` (Gỡ Khoá Sau Gate) HOÀN TẤT TRỌN VẸN (Tasks D1–D10 & LG1–LG5, 34.5 MD).** Hoàn tất toàn bộ 27 deliverables: Chốt ngưỡng $N\text{-}05 \ge 90.0\%$ (D1), nâng cấp 4 ACGs thành định nghĩa sản phẩm chính thức (D2), giải quyết 6 Open Items của 11 ADRs (D3), ban hành ADR-012 Key Custody Architecture (D4), đóng băng Capsule Format v1 trong ADR-002 & SDD §4 (D5), thiết kế authn/authz & CLI operational verbs (D6), gỡ `GATE-02` ban hành 5 Epics và 15 User Stories trong `022-User-Stories/` (D7), ban hành Master Test Plan V0.1 trong `035-QA/` (D8), nâng cấp Threat Model với L2 Container Sandbox (D9), ban hành ADR-013 OSS License Apache-2.0, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md`, và `SLA-Security-Response.md` (LG1–LG5). **CHÍNH THỨC SẴN SÀNG CHO GATE D10 (CẤP VỐN V0.1).** |
