...
| **Capsule Store** (system actor) | Trả danh sách capsule và nội dung capsule, sau khi kiểm tra quyền và giải mã | ⚠️ **Chưa được đặc tả** (`U-06`) |
...
| P4 | Capsule **chưa hết retention** | §20.5, §20.17 | `FR-024` |
...
**`TBD` — hai câu hỏi `RQ.md` không trả lời**:

1. **Thời hạn retention mặc định là bao nhiêu?** `RQ.md` chỉ nói *"configurable"* và không đưa giá trị mặc định nào. Một giá trị mặc định là quyết định compliance, **không tự bịa**.
2. **Capsule đã pull về máy local có bị retention chi phối không?** Đây là chỗ hở nghiêm trọng: `repro pull` (§8) tạo ra một **bản sao ngoài tầm kiểm soát của store**. Xoá ở phía server **không** xoá bản sao trên laptop developer — và §20.17 lại đòi hỗ trợ *deletion* cho mục đích compliance. `RQ.md` không nhắc tới nghịch lý này ở bất kỳ đâu.

> **Liên quan**: [NFR-Repro](../NFR-Repro.md) mục 5.4 đề xuất **crypto-shredding** (capsule mã hoá bằng key riêng từng capsule giữ phía server; xoá = phá key ⇒ mọi bản copy đã nằm trên laptop/chat/git lập tức thành ciphertext vô nghĩa) như cơ chế duy nhất biến việc này từ **bất khả hồi** thành **khả hồi**. Đánh đổi: **mất khả năng replay offline**. Đánh đổi này **chưa được ai cân** ⇒ gắn nhãn **"cần validate"**, không viết như đã chốt.
...
| `FR-024` | Retention policy cấu hình được + deletion | P4, `A2` |
...
| Tài liệu | Quan hệ |
|---|---|
| [PRD-Repro](../PRD-Repro.md) | Hợp đồng FR (mục 5.7); mục 10.4 (**M2** — ✅ đã chốt 2026-08-14: OSS core); mục 10.5 (`U-06` — Capsule Store chưa đặc tả) |
| [NFR-Repro](../NFR-Repro.md) | `N-13` *Determinism over magic* (neo của `A4`); mục 5.3 (capsule integrity), 5.4 (M2 ✅ đã chốt 2026-08-14; crypto-shred vẫn là đề xuất cần validate), 5.5 (redaction là hygiene control), 5.6 (ghi lại field đã redact) |
...
| [UC-02 — Replay Capsule Locally](./UC-02-Replay-Capsule-Locally.md) | Bước 1 của UC-02 chính là bước 1 của UC này; UC này giúp chẩn đoán `A5`/`A6` của UC-02 |
