...
| **Không có mã `SEC-xxx` nào phủ vế này** — neo vào **exit criteria `B2`**: vế **(c)** *cách ly tầng IAM*, nửa **`revoke/rotate`** của vế **(d)**, và nửa *"độc lập"* của vế **(b)**; cộng **`THREAT-018`** residual risk (b). Lens security đã grep toàn bộ threat model theo sáu từ khoá *IAM*, *least privilege*, *rotate*, *revoke*, *credential*, *destroy*: `SEC-017`–`SEC-020` nói về collector và capsule store, `SEC-032`–`SEC-036` nói về egress lúc replay — **không mã nào nói về thẩm quyền phá huỷ hạ tầng**. ⚠️ Sự vắng mặt đó là **một phần của mục ghi này**, không phải thiếu sót của nó: gán bừa một mã `SEC` sẽ làm task audit `B9` **xác minh một dòng SAI** | Phạm vi `P0-B`, task `B2` (⚙️ DevOps) — **không** pre-register: ghi **cùng PR** với code, theo quyết định `G-2` của gate 2026-08-15 (*"mô phỏng cục bộ + một dòng shortcut ledger"*) | **Ba thứ bị bỏ có ý thức, do `G-2` chốt mô phỏng cục bộ thay vì cloud thật.** **(1) Cách ly tầng IAM KHÔNG TỒN TẠI** — không phải *"làm yếu hơn"* mà là *"không tồn tại ở dạng cục bộ"*: `docker.sock` là **ambient authority tương đương root trên toàn daemon** — không principal, không scope, không deny boundary, không policy engine, và máy không có `vault`/`localstack`. Ai destroy được spike thì cũng destroy được `tnm_postgres` của dự án khác trên cùng daemon. Vế *"credential destroy không có quyền chạm ngoài scope"* được thay bằng **kỷ luật nhãn trong script** — một ràng buộc do **chính script tự áp lên mình** (filter nhãn duy nhất `repro.spike.env=<run_id>`, guard tiền tố `tnm_`, cấm `prune` mọi dạng), **không** phải một boundary do hệ thống cưỡng chế. **(2) Bằng chứng chỉ độc lập ở TẦNG CÔNG CỤ** (package riêng, không import destroy, gọi thẳng Docker Engine API cộng probe TCP từ host cộng `lsof`), **KHÔNG độc lập ở tầng thẩm quyền**: nguồn sự thật vẫn là chính `dockerd` mà destroy điều khiển, và máy có **3 docker context** — resource sống sót ngoài context được enumerate thì verifier **mù hoàn toàn**. Mệnh đề chứng minh được là *"trong scope nhãn, trên MỘT context, tại thời điểm đo, không còn resource khớp nhãn"*, **không phải** *"không tồn tại đường nào để môi trường gốc còn sống"*. **(3) Không revoke/rotate credential destroy** — docker socket **không có token để thu hồi, không có phiên để hết hạn**; điều khoản này là **no-op**, **không phải** *đã thoả* (rotate mật khẩu Postgres mỗi run là secret của **môi trường**, không phải thẩm quyền của **đường teardown**, và không đếm vào đây). **Hệ quả loopback — phải khai kèm, không chỉ khai *"không có IAM"***: vì không boundary nào tách spike khỏi phần còn lại của daemon và host, và vì allowlist `L2` **bao gồm loopback theo thiết kế** ([MTP-Spike-Phase-0](../035-QA/Test-Plans/MTP-Spike-Phase-0.md) §5.3 `T12`), một WRITE rò rỉ tới loopback có thể đáp xuống **dịch vụ THẬT của dự án khác** đang chạy trên cùng máy — `tnm_redis` giữ `6379`, `tnm_postgres` giữ `5433`, Up 7+ ngày — thay vì vào canary. `THREAT-018` residual risk (b) (*"đường loopback bị lạm dụng nếu máy developer có dịch vụ thật lắng nghe ở localhost"*) do đó chuyển từ **giả định** thành **điều kiện hiện hữu**. Hai hệ quả, **cả hai im lặng**: `escaped_side_effects` đọc ra `0` sai, và thiệt hại dữ liệu **ra ngoài blast radius**. Control bù: cơ chế **`canary_coverage` fail-closed** theo §4.6 — thiếu bất kỳ vế nào ⇒ `incomplete` ⇒ run tính **KHÔNG đạt**, và `canary_coverage` in **cạnh** `escaped_side_effects` trong cùng file máy đọc được. ⚠️ **KHÔNG nằm trong dòng này — được hiện thực ĐẦY ĐỦ**: destroy **theo nhãn**; **idempotent** (chứng minh bằng 2 lần chạy liên tiếp, so khối `assertions` của 2 file bằng chứng); **canary sink** (TCP accept log cộng HTTP log cộng DB sink Postgres `log_statement=all` với bảng audit INSERT-only). ⇒ **Mô phỏng cục bộ hạ giá trị BẰNG CHỨNG DESTROY, KHÔNG hạ BẰNG CHỨNG AN TOÀN** — `escaped_side_effects` lấy nguồn từ canary và canary hiện thực đủ; `C4` **không được gộp** hai thứ rồi hạ thấp cả hai | `src/spike/infra/` — `destroy.sh`, `verify/verify.js`, `coverage/coverage.js`, `docker-compose.spike.yml`, `docker-compose.canary.yml`; và mọi bằng chứng `docs/035-QA/Evidence/*.json` sinh từ chúng | 2026-08-15 |

> ⚠️ **Dòng thứ sáu KHÁC năm dòng trên về cách ghi.** Năm dòng đầu là **pre-register tại `A1`** (ngoại lệ được nêu tên ở §5.2). Dòng thứ sáu là mục ghi **đúng quy tắc chuẩn**: nó phát sinh tại `B2` và được ghi **cùng PR** với code của `B2` — cùng branch `spike/p0b-wave1`, cùng commit. Nó cũng là dòng đầu tiên **không neo được vào một mã `SEC-xxx`**, và lý do của điều đó nằm ngay trong cột 1.
