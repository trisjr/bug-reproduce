...
| API & Authn/Authz | ✅ **ĐÃ CHỐT TẠI PHASE P1 (Task D6)**: mTLS / Bearer API Token; RBAC (Developer read-only local capsules, SRE/Admin manage retention/purge). |
| CLI Operational Verbs | ✅ **ĐÃ CHỐT TẠI PHASE P1 (Task D6)**: `repro auth login/logout`, `repro purge --before=<date>`, `repro keys rotate/status`, `repro audit log`. |
| Key Custody Integration | ✅ **ĐÃ CHỐT TẠI PHASE P1 (Task D4 / [ADR-012](./ADR-012-Key-Custody.md))**: Tích hợp Private KMS / Vault quản lý DEK và thực thi crypto-shredding $SEC\text{-}016$. |
| Retention TTL | ✅ **CHỐT GATE-05a — 2026-08-14**: mặc định **30 ngày** (`SEC-022`), tự động crypto-shredding tại Key Custody Store khi hết hạn. |
...
|---|---|---|---|---|---|
| `U-01` | Cơ chế intercept PostgreSQL driver | §11, §18 (không nói cơ chế) | [ADR-007](./ADR-007-In-Process-SDK-Interception.md), [ADR-003](./ADR-003-Database-Record-Replay-Not-Snapshot.md) | ✅ **`CHỐT`** — Monkey-patching `pg.Client.prototype.query` ở tầng pure JS connection wrapper (Task D3). | §3.2 |
| `U-02` | **Định danh query để match lúc replay** | §6 (`query-001.json` ⇒ hàm ý theo thứ tự) | [ADR-003](./ADR-003-Database-Record-Replay-Not-Snapshot.md); `U-11` | ✅ **`CHỐT`** — Normalized SQL Fingerprint + Parameter Binding Array Matching (Task D3). | §4.4 |
| `U-03` | Cơ chế intercept HTTP (outbound **và inbound**) | §12, §18 (không phân biệt hai chiều) | [ADR-004](./ADR-004-Record-Replay-External-Inputs-At-Boundary.md), [ADR-007](./ADR-007-In-Process-SDK-Interception.md) | ✅ **`CHỐT`** — Outbound: `http/fetch` wrapper; Inbound: `http.Server` wrapper (Task D3). | §3.2 |
| `U-04` | **"execution path" và "sufficiently equivalent" nghĩa là gì** | §10 dùng `A → B → C` nhưng **không định nghĩa**; §10 cũng chính là chỗ in `✓ Execution matched` | [ADR-006](./ADR-006-Execution-Verification-By-Equivalence.md); §4.6; §5.3; `N-05` | ✅ **`CHỐT`** — Đã chốt chính thức qua $ACG\text{-}01$: Dãy `InteractionUnit` ($U_0 \dots U_\infty$) qua Rubric 2 tầng (Task D2). | §4.6, §5.3 |
| `U-05` | Versioning của capsule format | `RQ.md` **không nhắc tới ở đâu** | [ADR-002](./ADR-002-Repro-Capsule-Format-Contract.md) | ✅ **`CHỐT`** — Semantic versioning `"1.0.0"` trong `manifest.json` (Task D5). | §4.2 |
| `U-06` | Capsule Store: API, auth, backend, mô hình triển khai | §8/§18/§28/§20.6 hàm ý có store | §5.4; [ADR-009](./ADR-009-Private-Self-Hosted-Topology.md) | ✅ **`CHỐT`** — Sàn object storage + mTLS/API token + RBAC + CLI operational verbs (Task D6). | §5.4 |
| `U-06d` | **Key custody & crypto-shredding protocol** | §16, §20.5, `GATE-05b` | [ADR-012](./ADR-012-Key-Custody.md); [ADR-009](./ADR-009-Private-Self-Hosted-Topology.md) | ✅ **`CHỐT`** — Ban hành chính thức [ADR-012 Key Custody Architecture](./ADR-012-Key-Custody.md) (Task D4). | §4.9 |
| `U-07` | Capsule id là incident id hay id riêng của Repro | §8/§25 dùng `1842`; §2.1 có `Trace ID` | §4.8; tích hợp với observability (§34) | ✅ **`CHỐT`** — UUIDv7 cho capsule_id, lưu `trace_id`/`incident_id` trong metadata (Task D5). | §4.8 |
| `U-08` | **`verify` và `replay` cần hai bộ tiêu chí equivalence khác nhau** | `RQ.md` **không hề nói** | [ADR-006](./ADR-006-Execution-Verification-By-Equivalence.md) | ✅ **`CHỐT`** — `verify` kiểm tra sự vắng mặt của lỗi gốc và bảo toàn input bất biến (Task D3). | §3.9, §5.3 |
| `U-09` | **Nghịch lý capture trigger** | §20.7 *"capture only failed"* vs thực tế | [ADR-008](./ADR-008-Async-Bounded-Failure-Triggered-Capture.md) | ✅ **`CHỐT`** — Spike Phase 0 chứng minh Overhead buffer-discard chỉ $+1.62\%$ latency (Task D3). | §3.3 |
| `U-10` | `repro diff` có gọi dependency thật ở local không | §9 hiện `Local → tax = 12.43` mâu thuẫn §11/§12 | [ADR-011](./ADR-011-Execution-Diff-First-Class.md) | ✅ **`CHỐT`** — Replay-Interception Mode; toàn bộ WRITE bị chặn fail-closed theo [ADR-005](./ADR-005-Default-Deny-Write-Side-Effects.md) (Task D3). | §3.10 |
| `U-11` | Local phát ra interaction không có trong capsule | `RQ.md` **hoàn toàn không nêu** | §3.7; §5.3 | ✅ **`CHỐT`** — 6-step Divergence Attribution Protocol (Task D2/D3). | §3.7 |
| `U-12` | Phân loại READ/WRITE bằng cơ chế nào | §13 phân loại theo verb | [ADR-005](./ADR-005-Default-Deny-Write-Side-Effects.md) | ✅ **`CHỐT`** — Fail-closed 2 tầng ($L1$ Sink AST Filter + $L2$ Container Sandbox) (Task D3). | §7.3 |
| `U-13` | Clock: freeze tại một thời điểm hay virtual clock chạy tiếp | §18/§20.2 chỉ nói *"clock replay"* | [ADR-010](./ADR-010-Bounded-Determinism-Scope.md) | ✅ **`CHỐT`** — Virtual Clock Model tịnh tiến ảo tất định (Task D3). | §3.2 |
| `U-14` | Surface của feature flag | §5/§18 nói capture flag state | [ADR-004](./ADR-004-Record-Replay-External-Inputs-At-Boundary.md) | ✅ **`CHỐT`** — Capture giá trị boolean/string đã đánh giá tại thời điểm thực thi (Task D3). | §3.2 |
| `U-15` | **Redaction phá hỏng replay fidelity** | `RQ.md` §16 không nêu căng thẳng này | §9; Execution Diff | ✅ **`CHỐT`** — Format-preserving + ghi lại vết `redacted: true` (Task D2/D5). | §3.4, §7.2 |
| `U-16` | Phân tầng warning cho drift | §15/§20.8–20.10 chỉ có **một** mức | §3.11 | ✅ **`CHỐT`** — 3 tầng: Fatal (exit 1), Warning (exit 0 + ⚠️), Info (Task D3). | §3.11 |
| `U-17` | Nguồn của schema version | §15/§20.9 yêu cầu capture | §3.11 | ✅ **`CHỐT`** — Tự động đọc từ migration history table hoặc git commit hash (Task D3). | §3.11 |
| `U-18` | Hành vi khi capsule vượt size limit | §20.12 nói có *"size limits"* | §4.7; [ADR-002](./ADR-002-Repro-Capsule-Format-Contract.md) | ✅ **`CHỐT`** — Gắn cờ `truncated: true`, phân loại phân kỳ sang `truncated` (Task D5). | §4.7 |
| `U-19` | Replay Runtime là library hay process wrapper | §17 vẽ box, **không nói hình thái** | §3.7; `U-12` | ✅ **`CHỐT`** — Hỗ trợ cả hai: `npm install @repro/node` và `node --require @repro/node` (Task D3). | §3.7 |
| `U-20` | Thứ tự async trong **một** execution | §20.13 hoãn race condition | [ADR-006](./ADR-006-Execution-Verification-By-Equivalence.md) | ✅ **`CHỐT`** — Microtask tracking trong request lifecycle; race liên-request gán `out_of_scope` (Task D3). | §3.9 |
| `U-21` | Exit code CLI và output máy đọc được | `RQ.md` **không nói gì** | §6.5 CI integration | ✅ **`CHỐT`** — Hỗ trợ cờ `--json` và bảng exit code chuẩn hoá (Task D6). | §3.12 |
| `U-22` | Capsule format có chỗ chứa multi-service không | §26 đặt multi-service ở V0.3 | [ADR-002](./ADR-002-Repro-Capsule-Format-Contract.md) | ✅ **`CHỐT`** — Thêm trường `service_name` trong schema interaction v1 (Task D5). | §4.8 |
| `U-23` | Capsule format có language-agnostic không | §26 đặt Python/Go ở V0.3 | [ADR-002](./ADR-002-Repro-Capsule-Format-Contract.md) | ✅ **`CHỐT`** — UTF-8 Canonical JSON & Raw Byte Arrays (Task D5). | §3.13 |
| `U-24` | **Cơ chế phát hiện *"ngoài phạm vi determinism"*** | §33.5 đặt nghĩa vụ giải thích | [ADR-010](./ADR-010-Bounded-Determinism-Scope.md) | ✅ **`CHỐT`** — Tự động gắn nhãn `class_assessment` ($ACG\text{-}07$) và 6-step attribution (Task D2/D3). | §3.7, §5.3 |
| `U-25` | **Replay hai lần trên cùng một capsule có cho cùng kết quả không** | §23 đo *Execution Match Rate* | [ADR-006](./ADR-006-Execution-Verification-By-Equivalence.md) | ✅ **`CHỐT`** — Kiểm chứng $100\%$ tính tất định lặp lại qua $K=3$ tại Phase 0 (Task D3). | §3.9, §8.2 |
