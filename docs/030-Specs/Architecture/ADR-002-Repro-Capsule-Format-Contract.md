...
| A8 | **Capsule mã hoá bằng khoá tổ chức dùng chung** thay vì khoá riêng từng capsule | **[inferred]** | Loại (yếu — chưa validate): khoá dùng chung không cho phép xoá chọn lọc, nên không hỗ trợ được crypto-shredding. Xem `E12` ở §Consequences và §Open items. |
...
- **`E12` — Crypto-shredding: ràng buộc *được đề xuất*, nhãn "cần validate — đánh đổi với replay offline chưa được giải".** Cơ chế: mỗi capsule mã hoá bằng khoá riêng giữ phía server, `replay` lấy khoá just-in-time ⇒ xoá = phá khoá, và mọi bản copy trên laptop / Slack / git lập tức thành ciphertext vô nghĩa. Đây là cơ chế duy nhất được nêu để biến boundary `storage → laptop` từ **bất khả hồi** thành khả hồi, và là điều kiện thực tế để trả lời GDPR right-to-erasure. Đánh đổi chưa giải: **mất replay offline** và tăng độ phức tạp self-host. RQ.md không có nội dung này; không viết như đã chốt.
...
| **`E12`** | **Crypto-shredding** — ràng buộc **được đề xuất**, nhãn ***"cần validate — đánh đổi với replay offline chưa được giải"***. 📌 **M2 (✅ ĐÃ CHỐT 2026-08-14) KHÔNG chạm tới mục này** — quyết định đó chỉ nói về authn + authz + audit. Nhãn *cần validate* giữ nguyên. | Khoá riêng từng capsule giữ phía server, lấy just-in-time lúc replay; xoá = phá khoá. | Chặn câu trả lời GDPR right-to-erasure; chặn quyết định capsule có replay offline được hay không; chặn thiết kế key management của bản self-host. |
